locals {
  container_name = "green-blue-ecs-flask"
}
...
module "container_definition" {
  ...
  container_image = "${data.aws_ecr_repository.flask.repository_url}:latest"
...
data "aws_iam_policy_document" "execution_role" {
  statement {
    ...
    resources = ["${data.aws_ecr_repository.flask.arn}"]
  }
...
data "aws_iam_policy_document" "task_role" {
  statement {
    ...
    resources = ["${aws_ecs_cluster.flask.arn}"]
  }
}
...
resource "aws_ecs_service" "flask" {
  name            = "flask-service"
  task_definition = "${aws_ecs_task_definition.flask.id}"
  cluster         = "${aws_ecs_cluster.flask.arn}"
